...
- `codexer add <name> [--include-auth] [--include-config] [--open]`
- `codexer new <name> [--include-auth] [--include-config] [--open]`
- `codexer register <name> [--include-auth] [--include-config] [--open]`
  ...
  - Skips root-level `config.toml` unless `--include-config` is passed.
  - `--open` opens the created profile directory.
  - Errors if the profile already exists.
- `codexer init <name> [--include-auth] [--include-config] [--open] [codex args...]`
...
- `add_profile(name, include_auth=False, include_config=False, root=None, source_home=None) -> AddResult`
...
- Root-level `auth.json` and `config.toml` are the only default skipped files.
...
- Skipping and including `auth.json` and `config.toml`.
...
7. Verify:
   - `auth.json` and `config.toml` were skipped.
